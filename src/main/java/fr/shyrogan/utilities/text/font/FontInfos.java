 ...
 * more consistent on UTF-8 characters.
 * This enum is used inside of Text.
 *
 * @author SpirSpoodles on Spigot
 */
public enum FontInfos {
    ...
    FontInfos(char character, int length) {
    ...
    public int getBoldLength() {
        if(this == FontInfos.SPACE) return this.getLength();
     ...
     * @return FontInfos
     */
    public static FontInfos getFontInfo(char c) {
        // Removes Unicode characters.
        int pos = UNICODE.indexOf(c);
        if(pos > -1) c = PLAIN_ASCII.charAt(pos);

        for(FontInfos dFI : FontInfos.values()){
            if(dFI.getCharacter() == c) return dFI;
        }
        return FontInfos.DEFAULT;
    }
